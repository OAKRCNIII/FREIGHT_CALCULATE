...
function findFactoryPrice(priceSets: any[], factoryName: string, priceSetId?: string): {
  ...
  const ps = priceSetId
    ? priceSets.find((p) => p.id === priceSetId) || priceSets[priceSets.length - 1]
    : priceSets[priceSets.length - 1];
  const factoryKeys = Object.keys(ps.prices || {});

  // 1. exact match
  let matched = factoryKeys.find((k) => k === factoryName);
  // 2. case-insensitive contains
  if (!matched) matched = factoryKeys.find((k) => k.toLowerCase().includes(factoryName.toLowerCase()));
  // 3. user query contains factory key
  if (!matched) matched = factoryKeys.find((k) => factoryName.toLowerCase().includes(k.toLowerCase().slice(0, Math.min(k.length, 6))));

  if (!matched) {
    const available = factoryKeys.filter((k) => (ps.prices[k] || 0) > 0).slice(0, 20);
...
function listFactories(priceSets: any[]) {
  ...
  const latest = priceSets[priceSets.length - 1];
